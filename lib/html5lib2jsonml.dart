...
Object encodeToJsonML(Node node) {
  if (node is Document || node is DocumentFragment) {
    if (node.nodes.length > 1) {
      ...
      var nodes = List<Object>.filled(node.nodes.length + 1, "");
      // var nodes = List<Object>.empty(growable: true)
      //   ..length = node.nodes.length + 1;
      nodes[0] = "";
  ...
  if (node is Text /* TEXT_NODE */) {
    ...
    final output = List<Object>.filled(1 + (node.attributes.isNotEmpty ? 1 : 0) + node.nodes.length,"");
    // final output = List<Object>.empty(growable: true)
    //   ..length = (1 +
    //       (node.attributes.isNotEmpty ? 1 : 0) +
    //       node.nodes
    //           .length); // (1 + (node.attributes.isNotEmpty ? 1 : 0) + node.nodes.length, []);
    output[0] = node.localName as Object;
    var i = 1;
